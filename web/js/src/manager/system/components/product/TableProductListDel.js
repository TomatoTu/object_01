...
import {deleteProduct,recoveryProduct,deleteManyProduct,recoveryManyProduct} from '../../actions/ProductRecycleAction';
...
class TableProductList extends Component {
  ...
  handleRecoveryManyProduct(checked){
    
    if(checked.length == 0){
      window.alert("请选择需要还原的列表!");
      return false;
    }

    if(confirm('确定还原？')){
      this.props.recoveryManyProduct(checked);
      this.handleDeleteAfter();
    }
  }
  render() {
    const {productsCur,products} = this.changState();
  	const {system,deleteManyProduct,recoveryManyProduct,...actions} = this.props;
    ...
    return (
    	<div className="prd-list-table">
            <div className="tb-operate">
                ...
                <a href="javascript:void(0);" onClick={()=>::this.handleRecoveryManyProduct(checked)}>批量还原</a> 
            </div>
...
class Tr extends Component {
 // 删除
  ...
  render() {
    let {product,checkeds,deleteProduct,handleCheckBox,handleDeleteAfter,recoveryProduct} = this.props;
    ...
    return (
        <tr>
            ...
            <td><img className="prd_img" src={img}/></td>
            <td>{product.title}</td>
            <td>{product.categories?product.categories.name:'未分类'}</td>
            <td>
              <a href="javascript:void(0);" onClick={()=>recoveryProduct(product['id'],handleDeleteAfter())}>还原</a>
	            <a href="javascript:void(0);" onClick={()=>::this.handleDelete(product['id'])}>彻底删除</a>
            </td>
        </tr>
    )
  }
...
export default connect(system,{deleteProduct,deleteManyProduct,recoveryProduct,recoveryManyProduct})(TableProductList);